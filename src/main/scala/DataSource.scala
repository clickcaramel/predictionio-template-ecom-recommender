...
class DataSource(val dsp: DataSourceParams)
  ...
  private def propertiesRdd(sc: SparkContext, entityType: String): RDD[(String, PropertyMap)] = {
    val q = if (entityType == "product") {
      s"""
        select
          cast(id as text) as id, 'product/' || id as eventid, '{"rating":' || COALESCE((select avg(rating) from product_review where product_id = product.id), 0) || ',"boost":' || LEAST(array_length(regexp_split_to_array(trim(description), E'\\W+',''), 1) / 100.0, 1.0) || ',"categories":[' || COALESCE((SELECT STRING_AGG('"' || category.id || '"', ',') FROM category JOIN product_category_relation AS pcr ON pcr.category_id = category.id WHERE product.id = pcr.product_id), '') || '],"status":"' || published_status || '","language":"english","location":"USA","reward":' || product.fee || '}' as properties,
